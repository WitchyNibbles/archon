...
**Update 2026-09-23 (bundle reading at 2.1.280, not a run).** Opus 5.5 shipped, and the engine's alias table now reads `opus:{default:"claude-opus-5-5", ...}`, so every Archon `opus` route moves to 5.5 with no code change. Two differences from Opus 5 were checked against Archon: the default effort drops from `high` to `medium` (harmless, since every Opus route names its effort) and 5.5 rejects disabled thinking (Archon never disables it). An `ANTHROPIC_DEFAULT_OPUS_MODEL` override would re-route the alias, and a reviewer's `system/init` `model` field is the observation that would confirm it; none has been recorded at 2.1.280.
