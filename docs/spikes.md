...
|---|---|---|---|---|
| **S1** | Live literal name of the subagent tool and deny-as-catalog-removal | `claude -p --output-format stream-json --verbose --setting-sources "" --strict-mcp-config` with default tools; record `system/init.tools`. Then with `--disallowedTools Agent` and again with `--tools Read`. | The literal name is recorded — **`Task` at 2.1.278**, not `Agent`, which is an accepted deny-rule alias — a bare deny removes it from `init.tools`; `--tools` yields exactly the listed set. The probe records the spelling rather than asserting one. | P3 init-catalog check |
...
| **S9** | Session identity and resume | `--session-id U`; kill -9 mid-stream; `--resume U` from the same cwd; `--fork-session`. | `U` echoed in `init` and `result`. **Resume does not recall context and this is recorded as a FAIL, not worked around**: after `kill -9` no transcript exists and `--resume U` starts fresh at `num_turns: 1`. The round-trip of `U` is the whole independence proof; an interrupted reviewer becomes a new attempt. | P3 provenance, interrupted-review recovery |
| **S10** | `--max-turns` and `--max-budget-usd` enforcement | Tiny loop task with `--max-turns 1` and separately `--max-budget-usd 0.01`. | Record whether `error_max_turns`/`error_max_budget_usd` subtypes appear. **Both are enforced at 2.1.278.** A reviewer turn cap must then be generous (≥ 40): a low cap is exhausted exploring and returns a null payload that mimics a refusal. | P3 bounds |
