...
class Workspace:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise WorkspaceError(
                f"Archon needs a working Git repository at {requested}. "
        ...
        default = Path(os.environ.get("XDG_STATE_HOME") or Path.home() / ".local/state") / "archon"
        base = Path(state_root).expanduser() if state_root is not None else default
        if not base.is_absolute():
            raise WorkspaceError("Archon state_root / XDG_STATE_HOME must be an absolute path.")
        base = base.resolve()
        ...
        if any(_inside(self.state_dir.resolve(), tree) for tree in worktrees):
            raise WorkspaceError(
                "Private Archon state must be outside every implementation worktree; "
                "use the default XDG state directory or another external state_root."
            )
        if self.state_dir.is_symlink():
            raise WorkspaceError("Private Archon state cannot be a symlink.")
        self.state_dir.mkdir(parents=True, mode=0o700, exist_ok=True)
        if self.state_dir.stat().st_uid != os.getuid():
            raise WorkspaceError("Private Archon state must belong to the current user.")
    ...
    def ensure_branch(self, name: str | None = None) -> dict[str, Any]:
        ...
        if name is None and current.startswith("archon/"):
            return baseline
        name = name or f"archon/work-{secrets.token_hex(6)}"
        if name.startswith("-"):
        ...
        self._git(
            "symbolic-ref", "-m", "archon: select delivery branch", "HEAD", f"refs/heads/{name}"
        )
    ...
    def snapshot(
        ...
        *,
        ...
        inert = f"archon-review-inputs-{destination.name}"
        relocated = {
