...
def open_runtime(repo: str | Path, state_home: str | Path | None = None) -> Runtime:
    ...
    try:
        ...
        service = ArchonService(workspace, store=store, verification=runner)
...
def create_server(
    ...
    *,
    ...
    def runtime() -> Runtime:
        if current is None:
            raise RuntimeError("Archon MCP lifespan has not started")
        return current
            ...
            async def guarded(*args, **kwargs):
                try:
                    ...
                    if not isinstance(exc, (ValueError, KeyError, OSError)):
                        _LOG.exception("Archon tool failed: %s", function.__name__)
