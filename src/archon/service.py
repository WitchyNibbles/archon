...
class ArchonService:
    ...
    def start(self, goal: str, acceptance: Sequence[Any], tasks: Sequence[Any] = (),
        ...
        def prepare() -> tuple[RunSpec, dict]:
            ...
            delivery = self.workspace.ensure_branch(name=branch or f"archon/{spec.run_id}")
    ...
    async def verify(self, run_id: str | None = None) -> dict:
        ...
        if not self.verification:
            raise ServiceError("Verification execution adapter is unavailable; restore the configured Archon runtime automatically and retry.")
        current = self._candidate(run)
        if current.branch != run["spec"]["branch"]:
