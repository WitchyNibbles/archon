...
// carried before P1 — a shipped script pointing at the installed package's src/ tree.
//
...
//   • @witchynibbles/archon/src or archon/src  — installed-src path (src/ not shipped)
...
const FORBIDDEN = [
  {
    // Catches any archon package (scoped or legacy) referencing src/ — src/ is not
    // shipped in P1+ packages. Uses a composed regex to avoid the literal string
    // "node_modules/archon/" appearing in this source (which would trip the grep
    // regression guard that checks for unscoped package path references).
    pattern: new RegExp(["node_modules", "@witchynibbles", "archon", "src"].join("\\/") + "|" + ["node_modules", "archon", "src"].join("\\/")),
    label: "archon/src (installed-src path — src/ not shipped post-P1)",
  },
  {
