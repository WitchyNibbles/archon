...
function makeDeps(overrides: Partial<InteractiveStopHookDeps> = {}): InteractiveStopHookDeps {
  return {
    ...
    writeResumeRequest: async (_cwd, _req) => ".archon/work/daemon/interactive-resume-request.json",
...
describe("handleInteractiveStop", () => {
  it("writes resume-request when threshold is crossed and handoff is committed", async () => {
    ...
    const deps = makeDeps({
      writeResumeRequest: async (_cwd, req) => {
        capturedRequest = req;
...
describe("validateResumeRequest", () => {
  ...
  it("accepts a request exactly at the age boundary", () => {
    // exactly 300 seconds old → valid (boundary inclusive)
    const boundaryDate = new Date(Date.now() - 300_000).toISOString();
    const req: ResumeRequest = {
      schemaVersion: 1,
      mode: "fresh_run",
      runId: "run-001",
