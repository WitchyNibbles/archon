...
from archon.models import CheckSpec, Policy
from archon.workspace import SourceChangedError, Workspace, WorkspaceError
...
def test_branch_preserves_index_and_dirty_work(workspace: Workspace) -> None:
    ...
    created = workspace.ensure_branch("archon/delivery")
    after = workspace.provenance()
    assert created["branch"] == "archon/delivery"
...
def test_same_branch_and_collision_are_automatic(workspace: Workspace) -> None:
    git(workspace.root, "branch", "archon/existing")
    branch = workspace.ensure_branch("archon/existing")["branch"]
    assert branch.startswith("archon/existing-")
...
def test_git_hooks_fsmonitor_and_inherited_overrides_never_execute(
    ...
    workspace.ensure_branch("archon/safe")
    workspace.snapshot()
...
def test_state_is_private_external_and_xdg_aware(
    ...
    assert workspace.state_dir == tmp_path / "xdg-state" / "archon" / "repos" / workspace.repo_id
...
def test_only_explicit_generated_paths_are_excluded(git_repo: Path, tmp_path: Path) -> None:
    (git_repo / ".archon").mkdir()
    (git_repo / ".archon" / "source.py").write_text("source")
    (git_repo / ".archon" / "status.json").write_text("generated")
    workspace = Workspace(git_repo, tmp_path / "state", generated_paths=(".archon/status.json",))
    assert ".archon/source.py" in workspace.manifest()
    assert ".archon/status.json" not in workspace.manifest()
...
def test_detached_head_gets_delivery_branch_without_checkout(workspace: Workspace) -> None:
    ...
    assert info["branch"].startswith("archon/")
