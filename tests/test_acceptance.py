...
def start_delivery(service: ArchonService) -> str:
...
def claim_implementation(service: ArchonService, run_id: str, root: Path) -> None:
...
def repair_pricing(service: ArchonService, run_id: str, root: Path, source: str) -> None:
    service.task_update(run_id, "pricing", "implementing")
...
async def verify(service: ArchonService, runner: VerificationRunner, run_id: str) -> dict:
...
def test_fresh_consumer_failed_check_review_rejection_repair_and_current_branch(
    ...
    async def exercise() -> None:
        ...
        try:
            ...
            assert git(git_repo, "branch", "--show-current").decode().strip().startswith("archon/")
            ...
            observed = subprocess.run(
                [sys.executable, "-m", "archon", "--repo", str(git_repo),
